...
#
#   error case  ?other_site_IRI  :atSite hung off a :Site       -> reported
#   control     ?asset_IRI       :atSite on a :PumpStation     -> silent
#
#   ./run.sh            run it
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-32: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-32: $EXPECTED reported for $ERROR_CASE -- the property's domain is :PumpStation and the template hangs it off a :Site."
else
  echo "CT-32: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-32: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-32: $CONTROL not reported -- it carries the property on the class that declares it."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-32: FAIL -- findings nobody chose:" >&2
