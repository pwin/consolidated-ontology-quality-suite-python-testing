...
#
#   error case  asset-A2  a pump station with no :atSite -> reported
#   control     asset-A1  the same, with one            -> silent
#
#   ./run.sh            run it
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-17: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-17: $EXPECTED reported for $ERROR_CASE -- the agreed pattern requires every pump station to stand somewhere, and this one does not."
else
  echo "CT-17: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-17: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-17: $CONTROL not reported -- it carries its site link."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-17: FAIL -- findings nobody chose:" >&2
