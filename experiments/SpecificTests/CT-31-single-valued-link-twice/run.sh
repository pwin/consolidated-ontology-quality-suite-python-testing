...
#
#   error case  asset-A1  two :atSite links, from two mappings -> reported
#   control     asset-A2  one                                 -> silent
#
#   ./run.sh            run it
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-31: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-31: $EXPECTED reported for $ERROR_CASE -- two mappings each contributed a site link and they disagree."
else
  echo "CT-31: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-31: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-31: $CONTROL not reported -- it stands in one place."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-31: FAIL -- findings nobody chose:" >&2
