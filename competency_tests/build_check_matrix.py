...
def check_facts(check_id: str, registry: Dict[str, dict]) -> Tuple[str, str, str]:
    ...
    if entry:
        return (entry["title"], entry["default_severity"],
                "project-check" if check_id.startswith("CMP-") else "registry")
...
def render(registry: Dict[str, dict], observed: Dict[tuple, int]) -> str:
    ...
    add("Generated by `build_check_matrix.py`. One row per (competency test, check) pair: which "
        "check answers each competency test, what that check is according to the "
    ...
    add("```")
    add("")
    add("### Source -- where the check comes from")
    add("")
    add("`ontology-quality-suite` does not answer every test on its own, so a row's *Source* "
        "says what has to exist for its check to be reported at all. COMPETENCY_COVERAGE.md's "
        "\"How it is checked\" column uses the same words.")
    add("")
    add("| Source | Meaning |")
    add("|---|---|")
    used_sources = {source for _cover, entry in pairs
                    for source in [check_facts(entry[1], registry)[2]] if source}
    for kind, meaning in competency.KINDS.items():
        if kind in used_sources:
            add("| `{}` | {} |".format(kind, meaning))
    add("")
    add("### Run -- which pass produced it")
    add("")
    add("A *Run* is one pass over the fixtures, named so a finding can be traced back to the "
        "command that produced it -- the footnote on each row. The same keys label the per-run "
        "finding lists in COMPETENCY_COVERAGE.md. Several checks answer a test only in a "
        "particular run: `CNF-001` means nothing in `sketch`, where the mappings and the ontology "
        "still agree, and everything in `sketch-v2`, where they no longer do.")
    add("")
    add("| Run | What it is asked of | Produced by |")
    add("|---|---|---|")
    for spec in runspecs.RUNS.values():
        add("| `{}` | {} | {} |".format(spec.key, _cell(spec.title), spec.runner or "--"))
    add("")
    ...
    add("The tests pin {} of the registry's {} checks as evidence. The other {} still run in the "
    ...
    add("")
    add("*Category* is the registry's own grouping of its checks, which a check id's prefix "
        "mirrors: {}.".format(", ".join(
            "`{}-*` {}".format(prefix, category)
            for prefix, category in sorted(
                {check_id.split("-")[0]: entry["category"]
                 for check_id, entry in registry.items()}.items()))))
    add("")
