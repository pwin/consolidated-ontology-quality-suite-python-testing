...
#
#   error case  flow-S2  typed, with a unit, and no number -> reported
#   control     flow-S1  a number and a unit               -> silent
#
#   ./run.sh            run it
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-19: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-19: $EXPECTED reported for $ERROR_CASE -- the node was built and the source cell was empty, so it is a magnitude of nothing."
else
  echo "CT-19: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-19: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-19: $CONTROL not reported -- it carries its number."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-19: FAIL -- findings nobody chose:" >&2
