...
#
#   error case  ?site_IRI     used in CONSTRUCT, bound nowhere -> reported
#   control     ?reading_IRI  used and bound                   -> silent
#
#   ./run.sh            run it
...
# answerable before the mapping has ever run, which is the point of asking it
# here.
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-14: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-14: $EXPECTED reported for $ERROR_CASE -- it is used in the template and bound nowhere, so every triple that uses it is dropped for every row."
else
  echo "CT-14: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-14: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-14: $CONTROL not reported -- it is bound by a BIND in the same query."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-14: FAIL -- findings nobody chose:" >&2
