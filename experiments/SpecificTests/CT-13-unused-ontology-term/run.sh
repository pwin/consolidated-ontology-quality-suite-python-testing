...
#
#   error case  :DecommissionedSite  declared, never populated -> reported
#   control     :Site                declared and populated    -> silent
#
#   ./run.sh            run it
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-13: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-13: $EXPECTED reported for $ERROR_CASE -- the ontology declares it and nothing in the data is one."
else
  echo "CT-13: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-13: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-13: $CONTROL not reported -- the data populates it."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-13: FAIL -- findings nobody chose:" >&2
