...
#
#   error case  pressure-R1  a pressure in litres per second -> reported
#   control     flow-R1      a flow rate in litres per second -> silent
#
#   ./run.sh            run it
...
if [ ! -f "$RESULTS" ]; then
  echo "CT-21: FAIL -- the run wrote no $RESULTS" >&2
  exit 1
fi
...
if grep "^$EXPECTED," "$RESULTS" | grep -q "$ERROR_CASE"; then
  echo "CT-21: $EXPECTED reported for $ERROR_CASE -- litres measure volume, not force per unit area."
else
  echo "CT-21: FAIL -- $EXPECTED was not reported for $ERROR_CASE" >&2
  STATUS=1
fi

if grep "^$EXPECTED," "$RESULTS" | grep -q "$CONTROL"; then
  echo "CT-21: FAIL -- $EXPECTED was also reported for $CONTROL, which is correct." >&2
  STATUS=1
else
  echo "CT-21: $CONTROL not reported -- its unit measures its kind of quantity."
fi
...
if [ -n "$UNEXPECTED" ]; then
  echo "CT-21: FAIL -- findings nobody chose:" >&2
